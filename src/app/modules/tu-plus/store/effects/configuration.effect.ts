...
export class ConfigurationEffect {
  ...
  ConfigurationEffect: Observable<Action> = createEffect(() =>
    this._actions$.pipe(
      ofType(ConfigurationActionLoad),
      switchMap((action: any) => {
        return this._service.ConfigurationService(action.Type).pipe(
          take(1),
          map((data: IResConfiguration) => {
            if (!!data && data.success) {
              return ConfigurationActionSuccess({
                data,
              });
            }
            ...
            return ConfigurationActionFail({
