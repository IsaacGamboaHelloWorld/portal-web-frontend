...
import { DsStatesCardEnum } from '@app/shared/ds/ds-states-card/constants/ds-states-card.enum';
...
describe('ExtractsComponent', () => {
  ...
  beforeEach(() => {
    ...
    component.formProduct = new FormGroup({
  ...
  it('_mapSelectOption', () => {
    const fch = new Date('2018-12-01T00:00:00-05:00');
    const year = fch.getFullYear();
    const value = {
      label: `${'DICIEMBRE'.toLowerCase()} - ${year}`,
      value: StatementMock.periods[0],
    };

    (component as any)._mapSelectOption(StatementMock.periods);

    expect(component.dateAlias.value).toEqual(value);
  });
  ...
  it('retryLoad', () => {
    const spy = spyOn(component as any, '_loadPeriods');
    component.retryLoad();
    expect(spy).toHaveBeenCalled();
  });

  it('_loadPeriods without data', () => {
    const model = TestBed.get(ExtractsModel);
    const spy = spyOn(model, 'creationPeriodsLoad');

    (component as any)._loadPeriods();
  ...
  it('getCardStates', () => {
    const result = component.getCardStates;
    expect(result).toEqual(DsStatesCardEnum);
  });
  ...
  it('_mapStateExtracts when success is true', () => {
    const data: any = {
      account: {},
      data: '',
      base64: '',
    ...
    (component as any)._mapStateExtracts(data);

    expect(spyNot).toHaveBeenCalledWith(
      'DOCUMENTS.EXTRACTS.DETAIL.SUCCESS',
      true,
      ClassNotification.SUCCESS,
    );
  });

  it('_mapStateExtracts when success is true and downloaded is false', () => {
    const data: any = {
      account: {},
      data: '',
      base64: '',
  ...
  it('_mapStateExtracts when success is false', () => {
    const data: any = {
      account: {},
      data: '',
      base64: '',
  ...
  it('_mapStateExtracts when success is false and retryCount < maxAmountRetry', () => {
    const data: any = {
      account: {},
      data: '',
      base64: '',
  ...
  it('_mapPeriods with data', () => {
    const data = StatementMock;
    const result = (component as any)._mapPeriods(data);
    expect(result).toEqual(data.periods);
  });
      ...
      instance: {
        componentRef: {
          instance: {
            ...
            title: 'DOCUMENTS.EXTRACTS.MODAL_ERROR.TITLE',
            description: 'DOCUMENTS.EXTRACTS.MODAL_ERROR.TITLE',
            btnAgree: 'DOCUMENTS.EXTRACTS.MODAL_ERROR.BTN',
            actionAgree: of(''),
          },
        },
      },
